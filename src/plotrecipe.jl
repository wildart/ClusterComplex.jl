...
@recipe function f(mc::T, args...; pc=false) where {T<:MvNormal}
    χ = length(args) > 0 ? args[1] : 1.0
    B, V = elliptical_bound(cov(mc), χ)

    mu = mean(mc)
    if pc
        pts = [mu.+V[:,1] mu mu.-V[:,2]]
        @series begin
            seriestype := :path
            linewidth --> 2
            label := ""
...
@recipe function f(mcr::T, χ::Float64=1.0; colors=false) where {T<:ModelClusteringResult}
    for (i,mc) in enumerate(mcr.models)
        @series begin
            label --> "CB$i"
